      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <h2 className="font-serif text-xl text-amber-900 mb-1">{t('auth.enter_realm')}</h2>
            <p className="text-sm text-stone-500 mb-5">{t('auth.choose_path')}</p>

            <div className="space-y-3">
              {/* OAuth */}
              <button onClick={() => void signInWithGoogle()} className={`${btnOAuth} bg-white border-slate-300 text-slate-700 hover:bg-slate-50`}>
              ...
              {email.length > 0 && !emailValid && (
                <p className="text-xs text-red-500 -mt-2">{t('auth.invalid_email')}</p>
              )}
              <input
                type="password"
                value={password}
                ...
                className="w-full bg-amber-50/50 border-2 border-amber-800/20 rounded-sm px-4 py-2.5 text-sm text-amber-900 placeholder-stone-400 focus:outline-none transition-all"
              />
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-px bg-amber-800/15" />
                <span className="text-xs text-stone-400">{t('auth.or_email')}</span>
                <div className="flex-1 h-px bg-amber-800/15" />
              </div>
              <button
                onClick={() => { onClose(); onOpenRegister(); }}
                className="w-full border border-amber-700/40 text-amber-800 hover:bg-amber-100/50 text-sm py-2.5 rounded-sm transition-colors"
              >
