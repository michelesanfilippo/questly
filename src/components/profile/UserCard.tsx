  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="relative bg-[#faf7f0] border-2 border-amber-800/30 rounded-sm shadow-[2px_4px_12px_rgba(101,67,33,0.2)] p-5"
      >
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-serif font-bold text-amber-900 truncate">{profile.nickname}</span>
            <span className="inline-block px-2 py-0.5 text-xs rounded-full bg-amber-100 text-amber-800 w-fit mt-0.5">
        ...
        {/* XP bar */}
        <div className="h-2 w-full bg-amber-100 rounded-full overflow-hidden mb-1">
          <div
            className="h-full bg-gradient-to-r from-amber-400 to-amber-600 transition-all duration-500"
            style={{ width: `${xpPercent}%` }}
          />
        </div>
        <p className="text-xs text-stone-500 mb-1">{xpCurrent} / {xpNeeded} XP</p>
        <p className="text-xs text-stone-500">{t('user.quests_completed', { n: profile.missions_completed })}</p>

        {/* Badges */}
        {earnedBadges.length > 0 && (
          <>
            <div className="border-t border-amber-800/10 my-3" />
            <p className="text-xs uppercase tracking-widest text-stone-400 mb-2 font-semibold">{t('user.badges')}</p>
